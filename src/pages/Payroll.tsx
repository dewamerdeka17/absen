...
import { api, downloadCsv, post } from '../api'
    ...
    <>
      ...
      {loading ? <Busy /> : error ? <ErrorBox message={error} retry={reload} /> : !data?.run ? (
        ...
        <>
          ...
          <div className="inline-actions"><button className="button secondary" onClick={generate}><RefreshCw size={16} /> Hitung ulang</button><button className="button secondary" onClick={() => downloadCsv(`payroll-${month}.csv`, data.items)}><Download size={16} /> CSV</button></div></div>
